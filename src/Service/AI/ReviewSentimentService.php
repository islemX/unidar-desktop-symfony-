...
class ReviewSentimentService
{
    ...
    public function analyzeSet(array $reviews): array
    {
        ...
        return [
            ...
            'llm_summary'      => $this->llmSummarize($reviews),
        ];
    }
    ...
    private function llmSummarize(array $reviews): string
    {
        if (count($reviews) < 3) return '';
        $texts   = implode("\n---\n", array_map(fn($r) => $r['text'] ?? '', array_slice($reviews, 0, 10)));
        $prompt  = "Summarise these student housing reviews in 2 sentences, highlighting the main positives and issues:\n\n{$texts}";
        return $this->llm->complete($prompt, ['temperature' => 0.4, 'max_tokens' => 150]);
    }
}
